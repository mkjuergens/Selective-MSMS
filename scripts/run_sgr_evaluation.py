#!/usr/bin/env python3
"""
SGR (Selection with Guaranteed Risk) Evaluation.

Evaluates uncertainty measures for selective prediction with guaranteed risk control.
Separates retrieval (hit@k) and fingerprint loss evaluations.

Usage:
    python -m ms_uq.run_sgr_evaluation --config config/config_sgr.yml --group ensemble
"""
...
def load_data(pred_dir: Path, out_dir: Path, config: SGRConfig, loader=None):
    ...
    Pbits, _, _, ptr = load_predictions(
    ...
    scores_file = out_dir / f"scores_{config.metric}_{config.aggregation}.pt"
    if scores_file.exists() and not config.overwrite:
        ...
        data = scores_from_loader(Pbits, loader, metric=config.metric, aggregation=config.aggregation,
                                  ...
                                  return_per_sample=True, device=config.device)
