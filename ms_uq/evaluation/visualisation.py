...
def plot_rejection_curves(
    ...
    """
    Plot rejection curves showing performance vs rejection percentage.
    
    As more uncertain samples are rejected, remaining samples should
    have better performance if uncertainty is well-calibrated.
    
    Parameters
    ----------
    metrics : dict
        Uncertainty metrics, mapping name to (N,) array.
    losses : dict
        Loss values, mapping name to (N,) array.
    out_path : path-like, optional
        Output path. If None, displays plot interactively.
    title : str
        Figure title.
    selected_metrics : list of str, optional
        Subset of metrics to plot. If None, plots all.
    include_oracle : bool
        Whether to include oracle (perfect) rejection curve.
    include_random : bool
        Whether to include random rejection baseline.
    color_map : dict, optional
        Custom color mapping for metrics.
    confidence_metrics : list of str, optional
        Metrics that are confidence scores (higher = more confident).
        If None, auto-detected using is_confidence_score().
    figsize : tuple, optional
        Figure size (width, height).
    fontsize_title, fontsize_labels, fontsize_legend : int
        Font sizes for plot elements.
    y_label : str
        Y-axis label. If "auto", inferred from loss type.
    """
    sel = selected_metrics or list(metrics.keys())
    conf = confidence_metrics or [m for m in sel if is_confidence_score(m)]
    n = len(losses)
    figsize = figsize or (6 * n, 5)
    fig, axes = plt.subplots(1, n, figsize=figsize)
    colors = cm.viridis(np.linspace(0.1, 0.9, len(sel)))
    
    for idx, (name, vals) in enumerate(losses.items()):
        ax = axes[idx] if n > 1 else axes
        _setup_ax(ax)
        is_hit = name.startswith("hit")
        loss = (1.0 - vals if is_hit else vals).astype(np.float32)
        lt = torch.tensor(loss, dtype=torch.float32)
        
        if include_oracle:
            r, k = rejection_curve(lt, torch.tensor(loss, dtype=torch.float32))
            ax.plot(
                r.numpy(), (1.0 - k.numpy() if is_hit else k.numpy()),
                "k-", lw=3, label="Oracle", alpha=0.8
            )
        
        if include_random:
            ru = np.random.default_rng(42).random(len(loss)).astype(np.float32)
            r, k = rejection_curve(lt, torch.tensor(ru, dtype=torch.float32))
            ax.plot(
                r.numpy(), (1.0 - k.numpy() if is_hit else k.numpy()),
                "k--", lw=2, label="Random", alpha=0.5
            )
        
        for mi, mn in enumerate(sel):
            if mn not in metrics:
                continue
            unc = -metrics[mn].copy() if mn in conf else metrics[mn].copy()
            r, k = rejection_curve(lt, torch.tensor(unc, dtype=torch.float32), monotone=False)
            c = get_metric_color(mn, color_map) if color_map else colors[mi]
            ls, marker = get_metric_style(mn)
            ax.plot(
                r.numpy(), (1.0 - k.numpy() if is_hit else k.numpy()),
                label=mn, color=c, lw=2,
                ls=ls, marker=marker, markevery=5, markersize=5, alpha=0.9
            )
        
        ax.set_xlabel("% Rejected", fontsize=fontsize_labels)
        if idx == 0:
            ax.set_ylabel("Hit Rate" if is_hit else "Performance", fontsize=fontsize_labels)
        ax.set_title(_fmt_title(name), fontsize=fontsize_title)
        ax.set_xlim(0, 100)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=fontsize_legend, loc="best")
    
    if title:
        plt.suptitle(title, fontsize=fontsize_title + 2, y=1.02)
    plt.tight_layout()
...
_HEATMAP_ORDER = [
    ...
    "retrieval_aleatoric", "retrieval_total",
    "retrieval_epistemic",
    ...
    # External / Distance
    "n_candidates",
    "knn_distance", "mahalanobis", "relative_mahalanobis",
    "centroid_distance",
    # SpecBridge
    "entropy", "ambiguity_ratio",
]

_HEATMAP_GROUPS = {
    "Retrieval-level":   ["confidence", "score_gap", "retrieval_aleatoric",
                          "retrieval_total", "retrieval_epistemic",
                          "rank_var_1", "rank_var_5", "rank_var_20"],
    "Fingerprint-level": ["bitwise_total", "bitwise_aleatoric",
                          "bitwise_epistemic", "bitwise_epistemic_active"],
    "External":          ["n_candidates", "knn_distance", "mahalanobis",
                          "relative_mahalanobis", "centroid_distance",
                          "entropy", "ambiguity_ratio"],
}
...
def plot_aurc_comparison_horizontal(
    aurc_df: pd.DataFrame,
    out_path: Optional[Union[str, Path]] = None,
    title: str = "AURC Comparison",
    **kwargs,
) -> None:
    """Plot AURC as horizontal bars. Wrapper around plot_aurc_bars."""
    plot_aurc_bars(aurc_df, list(aurc_df.columns), Path(out_path or "aurc.png"), title=title)


def plot_aurc_comparison(
    aurc_df: pd.DataFrame,
    ...
    **kwargs,
    ...
    """
    Plot AURC comparison with configurable orientation.
    
    Parameters
    ----------
    aurc_df : pd.DataFrame
        AURC values with measures as index, losses as columns.
    out_path : path-like
        Output file path.
    title : str
        Figure title.
    orientation : str
        "horizontal" for bar chart, "vertical" for grouped bars.
    """
    if orientation == "horizontal":
        plot_aurc_bars(aurc_df, list(aurc_df.columns), Path(out_path), title=title)
    else:
        _plot_aurc_vertical(aurc_df, out_path, title, **kwargs)


def _plot_aurc_vertical(
    aurc_df: pd.DataFrame,
    out_path: Union[str, Path],
    title: str = "AURC Comparison",
    color_map: Optional[Dict[str, str]] = None,
    **kwargs,
    ...
    """Plot AURC as vertical grouped bars."""
    hrs = aurc_df.columns.tolist()
    mets = aurc_df.index.tolist()
    colors = [get_metric_color(m, color_map) for m in mets]
    
    fig, ax = plt.subplots(figsize=(max(10, 2 + len(hrs) * 1.5), 6))
    _setup_ax(ax)
    ...
    for i, m in enumerate(mets):
        ax.bar(
            x + (i - len(mets) / 2 + 0.5) * w,
            [aurc_df.loc[m, h] for h in hrs],
            w, label=m, color=colors[i], alpha=0.9, edgecolor="white", lw=0.5
        )
    
    ax.set_xlabel("Hit Rate", fontsize=13)
    ax.set_ylabel("AURC (lower = better)", fontsize=13)
    ax.set_title(title, fontsize=14, pad=15)
    ax.set_xticks(x)
    ax.set_xticklabels([h.replace("hit@", "Hit@") for h in hrs], fontsize=12)
...
def plot_aurc_by_aggregation(
    aurc_results: Dict[str, pd.DataFrame],
    out_path: Optional[Union[str, Path]] = None,
    title: str = "AURC by Aggregation Method",
    loss_key: str = "hit@1",
    figsize: Tuple[float, float] = (12, 6),
    dpi: int = 150,
) -> plt.Figure:
    """
    Compare AURC across different aggregation methods.
    
    Parameters
    ----------
    aurc_results : dict
        Mapping from aggregation name to AURC DataFrame.
    out_path : path-like, optional
        Output path. If None, returns figure without saving.
    title : str
        Figure title.
    loss_key : str
        Which loss column to compare (e.g., "hit@1").
    figsize : tuple
        Figure size.
    dpi : int
        Output resolution.
    ...
    ans = list(aurc_results.keys())
    
    ams = set()
    for df in aurc_results.values():
        if loss_key in df.columns:
            ams.update(df.index.tolist())
    ...
    for i, a in enumerate(ans):
        ...
        if loss_key not in df.columns:
            continue
        vals = [df.loc[m, loss_key] if m in df.index else 0 for m in meas]
        ax.bar(x + i * w, vals, w, label=a, color=cols[i % len(cols)], alpha=0.8)
    
    ax.set_xlabel("Uncertainty Measure", fontsize=12)
    ax.set_ylabel("AURC (lower = better)", fontsize=12)
    ax.set_title(title, fontsize=14)
    ax.set_xticks(x + w * (len(ans) - 1) / 2)
    ax.set_xticklabels(meas, rotation=45, ha="right", fontsize=10)
...
# =============================================================================
# Member vs Aggregate Aliases
# =============================================================================

def plot_member_vs_aggregate(
    mh: Dict[str, np.ndarray],
    ah: Dict[str, Dict[str, float]],
    out_path: Optional[Union[str, Path]] = None,
    title: str = "",
    **kwargs,
) -> None:
    """Alias for plot_member_vs_agg with optional output."""
    if out_path:
        plot_member_vs_agg(mh, ah, Path(out_path), title)


def plot_member_vs_aggregation(
    results: Dict[str, Dict[int, float]],
    figsize: Tuple[float, float] = (10, 6),
    save_path: Optional[Union[str, Path]] = None,
) -> plt.Figure:
    """
    Plot member vs aggregation from a results dictionary.
    
    Parameters
    ----------
    results : dict
        Keys like "Member_0", "Member_1", "score", etc.
        Values are dicts mapping k (int) to hit rate.
    figsize : tuple
        Figure size.
    ...
    """
    ks = [1, 5, 20]
    mn = [k for k in results.keys() if k.startswith("Member")]
    an = [k for k in results.keys() if not k.startswith("Member")]
    mh = {f"hit@{k}": np.array([results[m].get(k, 0) for m in mn]) for k in ks}
    ah = {a: {f"hit@{k}": results[a].get(k, 0) for k in ks} for a in an}
    
    if save_path:
        plot_member_vs_agg(mh, ah, Path(save_path), "")
    return plt.gcf()


# =============================================================================
# SGR (Selective Guaranteed Risk) Plots
# =============================================================================
...
def plot_sgr_comparison(
    ...
    """
    Generate SGR comparison plots from evaluation results.
    
    Creates two types of plots:
    1. Risk-coverage trade-off curves
    2. Coverage comparison bar charts
    
    Parameters
    ----------
    results : dict
        Mapping from result type to DataFrame.
        Keys starting with "table_" contain SGR table data.
        Keys starting with "comparison_" contain comparison data.
    out_dir : Path
    ...
    table_dfs = [df for k, df in results.items() if k.startswith("table_")]
    comp_dfs = [df for k, df in results.items() if k.startswith("comparison_")]
    
    if not table_dfs and not comp_dfs:
        return
    
    # Risk-coverage curves
    if table_dfs:
        comb = pd.concat(table_dfs, ignore_index=True)
        hms = comb["hit_metric"].unique()
        ams = comb["metric"].unique()
        colors = {m: cm.tab10(i / len(ams)) for i, m in enumerate(ams)}
        
        fig, axes = plt.subplots(1, len(hms), figsize=(6 * len(hms), 5), sharey=True)
        for ax, hn in zip(_axes_list(axes, len(hms)), hms):
            dh = comb[comb["hit_metric"] == hn]
            for m in ams:
                dm = dh[(dh["metric"] == m) & dh["feasible"]]
                if len(dm):
                    ax.plot(
                        dm["target_risk"], dm["coverage"],
                        "o-", color=colors[m], label=m, lw=2, ms=6
                    )
            ax.set_xlabel("Target Risk r*", fontsize=12)
            ax.set_ylabel("Coverage", fontsize=12)
            ax.set_title(hn.replace("hit@", "Hit@"), fontsize=13)
            ax.set_xlim(0, comb["target_risk"].max() * 1.05)
            ax.set_ylim(0, 1.05)
            ax.grid(alpha=0.3)
            ax.legend(fontsize=9, loc="lower right")
        
        plt.suptitle(f"{label}: Risk-Coverage Trade-off", fontsize=14)
        plt.tight_layout()
        plt.savefig(out_dir / "sgr_risk_coverage_curves.png", dpi=300, bbox_inches="tight")
        plt.close()
    
    # Coverage comparison
    if comp_dfs:
        comb = pd.concat(comp_dfs, ignore_index=True)
        hms = comb["hit_metric"].unique()
        
        fig, axes = plt.subplots(1, len(hms), figsize=(7 * len(hms), 6))
        for ax, hn in zip(_axes_list(axes, len(hms)), hms):
            dh = comb[comb["hit_metric"] == hn]
            df = dh[dh["feasible"]].sort_values("coverage", ascending=True)
            
            if not len(df):
                ax.text(
                    0.5, 0.5, "No feasible solutions",
                    ha="center", va="center", transform=ax.transAxes
                )
                continue
            
            colors = cm.RdYlGn(df["coverage"].values)
            ax.barh(range(len(df)), df["coverage"].values, color=colors, edgecolor="gray")
        ...
        plt.suptitle(f"{label}: SGR Coverage Comparison", fontsize=14)
        plt.tight_layout()
        plt.savefig(out_dir / "sgr_coverage_comparison.png", dpi=300, bbox_inches="tight")
        plt.close()


def plot_sgr_generalization(
    split_results: pd.DataFrame,
    out_dir: Path,
    label: str,
) -> None:
    """
    Plot calibration vs test coverage for SGR generalization analysis.
    
    Parameters
    ----------
    split_results : pd.DataFrame
        DataFrame with columns: hit_metric, uncertainty, cal_coverage, test_coverage.
    out_dir : Path
        Output directory.
    label : str
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    hms = split_results["hit_metric"].unique()
    mets = split_results["uncertainty"].unique()
    colors = {m: cm.tab10(i % 10) for i, m in enumerate(mets)}
    
    fig, axes = plt.subplots(1, len(hms), figsize=(6 * len(hms), 5))
    for ax, hn in zip(_axes_list(axes, len(hms)), hms):
        dh = split_results[split_results["hit_metric"] == hn]
        for m in mets:
            dm = dh[dh["uncertainty"] == m]
            if len(dm):
                ax.scatter(
                    dm["cal_coverage"], dm["test_coverage"],
                    c=[colors[m]], label=m, s=50, alpha=0.7
                )
        ax.plot([0, 1], [0, 1], "k--", alpha=0.5, label="Perfect")
        ax.set_xlabel("Calibration Coverage", fontsize=12)
        ax.set_ylabel("Test Coverage", fontsize=12)
        ax.set_title(hn.replace("hit@", "Hit@"), fontsize=13)
        ax.set_xlim(0, 1.05)
        ax.set_ylim(0, 1.05)
        ax.grid(alpha=0.3)
    ...
    plt.suptitle(f"{label}: Generalization", fontsize=14)
    plt.tight_layout()
    plt.savefig(out_dir / "sgr_generalization_scatter.png", dpi=300, bbox_inches="tight")
    plt.close()


# =============================================================================
# Advanced Risk-Coverage Plots
# =============================================================================

def plot_risk_coverage_with_oracle(
    losses: Dict[str, np.ndarray],
    uncertainties: Dict[str, np.ndarray],
    out_path: Optional[Union[str, Path]] = None,
    title: str = "",
    loss_key: str = "hit@1",
    selected_uncertainties: Optional[List[str]] = None,
    coverage_direction: str = "descending",
    figsize: Tuple[float, float] = (12, 5),
    include_auroc_panel: bool = True,
    dpi: int = 150,
) -> plt.Figure:
    """
    Plot risk-coverage curve with oracle baseline and optional AUROC panel.
    
    Parameters
    ----------
    losses : dict
        Mapping from loss name to (N,) array.
    uncertainties : dict
        Mapping from uncertainty name to (N,) array.
    out_path : path-like, optional
    ...
    loss_key : str
        Which loss to plot.
    selected_uncertainties : list of str, optional
        Which uncertainties to include. If None, auto-selects.
    coverage_direction : str
        "descending" (100% → 0%) or "ascending" (0% → 100%).
    figsize : tuple
        Figure size.
    include_auroc_panel : bool
        Whether to add a side panel showing AUROC for each uncertainty.
    dpi : int
        Output resolution.
    ...
    """
    lr = losses[loss_key]
    loss = (1.0 - lr if loss_key.startswith("hit") else lr).astype(np.float32)
    n = len(loss)
    su = selected_uncertainties or [k for k in uncertainties if k.startswith(("unc_", "std_"))]
    
    np_ = 2 if include_auroc_panel else 1
    fig, axes = plt.subplots(1, np_, figsize=figsize)
    axes = _axes_list(axes, np_)
    ax1 = axes[0]
    
    if coverage_direction == "descending":
        cov = np.linspace(1.0, 0.02, 100)
    else:
        cov = np.linspace(0.02, 1.0, 100)
    ...
    for c in cov:
        k = int(n * c)
        if k > 0:
            sel = order[-k:] if coverage_direction == "descending" else order[:k]
            orisks.append(loss[sel].mean())
        else:
            orisks.append(0)
    ax1.plot(cov, orisks, "k-", lw=2.5, label="Oracle")
    ax1.axhline(loss.mean(), color="gray", ls="--", alpha=0.7, label="Random")
    
    colors = plt.cm.tab10.colors
    auroc_data = []
    
    for i, key in enumerate(su):
        if key not in uncertainties:
            continue
        unc = uncertainties[key]
        valid = ~np.isnan(unc)
        if valid.sum() < 10:
            continue
        
        si = np.argsort(unc[valid])
        lv = loss[valid]
        nv = len(si)
        risks = [lv[si[:max(1, int(nv * c))]].mean() if int(nv * c) > 0 else 0 for c in cov]
        
        lbl = key.replace("unc_", "").replace("std_", "")
        col = colors[i % len(colors)]
        ax1.plot(cov, risks, color=col, lw=1.5, label=lbl)
        
        try:
            from sklearn.metrics import roc_auc_score
            auroc_data.append((lbl, roc_auc_score(loss[valid] > 0, unc[valid]), col))
        except Exception:
            pass
    
    ax1.set_xlabel("Coverage", fontsize=12)
    ylabel = f"Error Rate ({loss_key})" if loss_key.startswith("hit") else f"Risk ({loss_key})"
    ax1.set_ylabel(ylabel, fontsize=12)
    ax1.set_title(f"{title} Risk-Coverage" if title else "Risk-Coverage", fontsize=14)
    ax1.set_xlim((1, 0) if coverage_direction == "descending" else (0, 1))
    ax1.set_ylim(0, min(1, loss.mean() * 1.5))
    ax1.legend(loc="upper left" if coverage_direction == "descending" else "upper right", fontsize=9)
    ax1.grid(alpha=0.3)
    
    if include_auroc_panel and auroc_data:
        ax2 = axes[1]
        lbls, aucs, cols = zip(*auroc_data)
        y = np.arange(len(aucs))
        ax2.barh(y, aucs, color=cols, alpha=0.7)
        ax2.set_yticks(y)
        ax2.set_yticklabels(lbls, fontsize=9)
        ax2.axvline(0.5, color="black", ls="--", alpha=0.5)
        ax2.set_xlabel("AUROC", fontsize=12)
        ax2.set_title("Uncertainty Quality", fontsize=14)
        ax2.set_xlim(0.4, max(0.85, max(aucs) + 0.05))
...
def plot_meta_rejection_curves(
    test_metrics: Dict[str, np.ndarray],
    test_hit_rates: Dict[str, np.ndarray],
    target_hits: List[str],
    out_path: Union[str, Path],
    baseline_metrics: List[str] = None,
) -> None:
    """
    Plot rejection curves comparing meta-predictors against baselines.
    
    Parameters
    ----------
    test_metrics : dict
        Uncertainty metrics including meta-predictor outputs.
    test_hit_rates : dict
        Hit rates for each target (e.g., "hit@1", "hit@5").
    target_hits : list of str
        Which hit metrics to plot.
    out_path : path-like
        Output file path.
    baseline_metrics : list of str, optional
        Baseline uncertainty metrics to compare against.
    """
    if baseline_metrics is None:
        baseline_metrics = [
            "entropy_rel", "n_candidates", "retrieval_total",
            "retrieval_aleatoric", "bitwise_total"
        ]
    
    ns = len(target_hits)
    fig, axes = plt.subplots(1, ns, figsize=(8 * ns, 6), sharey=True)
    
    cp = cm.viridis(np.linspace(0.1, 0.9, len(baseline_metrics) + 1))
    colors = {m: cp[i] for i, m in enumerate(baseline_metrics)}
    
    nice = {
        "entropy_rel": "Entropy (rel)",
        "n_candidates": "N_candidates",
        "retrieval_total": "Retrieval TU",
        "retrieval_aleatoric": "Retrieval AU",
        "bitwise_total": "Bitwise TU",
    }
    
    for idx, hn in enumerate(target_hits):
        ax = axes[idx] if ns > 1 else axes
        lt = 1.0 - test_hit_rates[hn]
        
        # Baseline curves
        for mn in baseline_metrics:
            if mn not in test_metrics:
                continue
            unc = -test_metrics[mn] if "prob" in mn or "margin" in mn else test_metrics[mn]
            r, k = rejection_curve(torch.tensor(lt), torch.tensor(unc), monotone=False)
            ax.plot(
                r.numpy(), 1.0 - k.numpy(),
                label=nice.get(mn, mn), ls="--", lw=2,
                color=colors[mn], alpha=0.8
            )
        
        # Meta-predictor curve
        mk = f"meta_gb_{hn.replace('@', '')}"
        if mk in test_metrics:
            r, k = rejection_curve(
                torch.tensor(lt), torch.tensor(test_metrics[mk]), monotone=False
            )
            ax.plot(
                r.numpy(), 1.0 - k.numpy(),
                label=f"Meta-GB ({hn})", ls="-", lw=3.5,
                color=cp[-1], alpha=1.0
            )
        
        ax.set_xlabel("% Rejected", fontsize=14)
        ax.set_ylabel(hn.replace("hit@", "Hit@"), fontsize=14)
        ax.set_title(hn.replace("hit@", "Hit@"), fontsize=14)
        ax.set_ylim(0, 1)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=12, loc="upper left")
    
    plt.suptitle("Meta-Predictors vs Baselines", fontsize=16, y=1.00)
    plt.tight_layout()
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
...
# ── NEW: plot_single_loss_paired ─────────────────────────────────────

def plot_single_loss_paired(
    losses: Dict[str, np.ndarray],
    uncertainties: Dict[str, np.ndarray],
    aurc_df: pd.DataFrame,
    loss_col: str,
    ...
    """Side-by-side: RC curve (left) + AURC bars (right) for one loss column.

    Useful for isolating a single metric (e.g. tanimoto_loss) from saved
    evaluation outputs.  Bar panel height adapts to number of bars.
    """
    all_measures = [m for m in (measures or list(uncertainties.keys()))
                    if m in uncertainties]
    ...
    n_bars = len(aurc_df)
    panel_w = _ROW_W / 3  # one panel = 1/3 of the 3-panel row width
    bar_w = panel_w * 1.2  # bars slightly wider for label room
    leg_w = _LEG_W

    # Dynamic height based on bar count
    fig_h = max(_RC_H, _BAR_PITCH * n_bars + _BAR_PAD)

    fig = plt.figure(figsize=(panel_w + bar_w + leg_w, fig_h))
    gs = fig.add_gridspec(
        1, 3,
        width_ratios=[panel_w, bar_w, leg_w],
        wspace=0.35,
    ...
    _setup_ax(ax_rc)
    if loss_col in losses:
        lv = losses[loss_col]
        cov = np.linspace(1.0, 0.01, 100)
        N = len(lv)

        order = np.argsort(-lv)
        ax_rc.plot(cov, [lv[order[-max(1, int(N * c)):]].mean() for c in cov],
                   color=DEFAULT_COLOR_MAP["oracle"], lw=2.5)
        ax_rc.axhline(lv.mean(), color=DEFAULT_COLOR_MAP["random"],
                      ls="--", alpha=0.9, lw=2.5)
        ...
        for i, m in enumerate(all_measures):
            ...
            lval, nv = lv[valid], len(si)
            ls, marker = get_metric_style(m)
            ax_rc.plot(cov, [lval[si[:max(1, int(nv * c))]].mean() for c in cov],
                       color=get_metric_color(m), lw=2.3,
                       ls=ls, marker=marker, markevery=10, markersize=6, alpha=0.9)

    ax_rc.set_xlabel("Coverage", fontsize=_FL)
    ax_rc.set_xlim(1, 0)
    ax_rc.set_ylabel("Error Rate" if loss_col.startswith("hit@") else "Loss",
                     fontsize=_FL)
    ax_rc.set_title(_fmt_title(loss_col), fontsize=_FH)
    ax_rc.grid(alpha=0.3)
    ax_rc.tick_params(labelsize=_FT)
    _setup_rc_ticks(ax_rc)

    # ── Right: AURC bars ──
    ax_bar = fig.add_subplot(gs[0, 1])
    if loss_col in aurc_df.columns:
        present = [m for m in aurc_df.index if pd.notna(aurc_df.loc[m, loss_col])]
        vals = aurc_df.loc[present, loss_col].sort_values(ascending=True)
        _format_bar_panel(ax_bar, vals, loss_col)
